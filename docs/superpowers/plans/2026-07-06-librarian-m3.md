...
- **19.** Task 4, `test_html.test_unknown_tag_counted` — тот же класс SyntaxError-бага плана (bytes-литерал с кириллицей `b'<main>…подпись к рисунку…</main>'`). Фикс: `'<…>'.encode("utf-8")` от str-литерала, байт-в-байт идентичен задумке (112 байт, проверено судьёй побайтовым сравнением). Реализация `html.py` не тронута. Класс идентичен M2/13 и M3/18.
- **20.** Task 4, `test_html.test_empty_content_raises` — verbatim-фикстура `<nav><a href="/">x</a></nav>` эмпирически давала непустой XML `<main><p>x</p></main>` (trafilatura вытягивала текст ссылки «x» как контент) → `BrokenFileError` не поднимался → verbatim-тест падал `DID NOT RAISE`. Это каприз trafilatura 2.1 на синтетике, предусмотренный Task 4 Step 4 (мандат менять тестовые данные, не маппер). Минимальный фикс: `<a href="/"></a>` (пустая ссылка) → trafilatura честно возвращает None → `BrokenFileError("…основной контент")` поднимается. `_walk`/`extract` не тронуты; ассерт `match="основной контент"` сохранён. Воспроизведено судьёй независимо (обе версии прогнаны через trafilatura напрямую: verbatim → xml не None; фикс → xml is None).
